...
pokemon.delete("/:id([0-9]{1,3})", async (req, res, next) => {
    ...
    if(rows.affectedRosw == 1){
        return res.status(200).jason({ code: 200, message: "Pokemon borrado correctamente"});
    }
    return res.status(404).jason({ code: 404, message: "Pokemon no encontrado"});
});
...
pokemon.get('/:id([0-9]{1,3})', async (req, res, next) => {
    ...
    if(id >= 1 && id <= 722) {
        ...
        return res.status(200).jason({code: 200, message: pkmn });
    }
